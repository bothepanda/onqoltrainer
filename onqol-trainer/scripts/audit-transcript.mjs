...
const DISCLOSURE = [
  ...
  ["анионная разница (готовый расчёт)", /анионн\S* (разниц\S*|промежут\S*)[^.]{0,40}=|138\s*[−-]\s*110/i, /анион/i],
  ["ОПП/KDIGO (готовая трактовка)", /kdigo|повреждени\S* почек|(?<![\p{L}])опп(?![\p{L}])/iu, /опп|kdigo|повреждени/i],
  // Найдено вручную (TH1, 22.09.2026): «анионный промежуток... гиперхлоремический компонент» без метки <<HINT>> —
  // готовый диагноз/механизм, а не запрошенные данные. inRequest широкий: если резидент этот термин уже
  // произнёс сам, это не лишняя выдача.
  ["гиперхлоремический ацидоз (готовая трактовка)", /гиперхлоремич/i, /гиперхлоремич|хлорид[^.]{0,20}ацидоз|ацидоз[^.]{0,20}хлорид/i],
  ["гиповолемия/дегидратация — диагноз назван преподавателем", /гиповолеми|дегидратаци/i, /гиповолеми|дегидратаци|обезвож/i],
];
