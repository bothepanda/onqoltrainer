...
const C = {
  ...
  text: "#0d2124", mid: "#2c5f68", sub: "#4f7f87", border: "rgba(22,105,122,0.18)",
...
const S = {
  ...
  logo: { fontWeight: 700, letterSpacing: "0.12em", color: C.accent, fontSize: 16 },
  ...
  h1: { fontSize: 22, margin: "0 0 12px", fontWeight: 600 },
  btn: { background: C.accent, color: "#fff", border: 0, borderRadius: 8, padding: "10px 16px", fontSize: 16, cursor: "pointer", fontFamily: "inherit" },
  btn2: { background: "transparent", color: C.accent, border: `1px solid ${C.border}`, borderRadius: 8, padding: "9px 14px", fontSize: 15, cursor: "pointer", fontFamily: "inherit" },
  input: { width: "100%", boxSizing: "border-box", padding: "10px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 18, fontFamily: "inherit", background: "#fff", color: C.text },
  label: { fontSize: 14, color: C.mid, display: "block", margin: "12px 0 4px" },
  err: { background: C.errorBg, color: C.error, border: `1px solid ${C.error}33`, borderRadius: 8, padding: "8px 12px", fontSize: 15, margin: "10px 0" },
};
  ...
  return (
    <div style={S.page}>
      <div style={S.header}>
        ...
        <span style={{ fontSize: 14, color: C.sub, flex: 1 }}>учебные сессии</span>
        {resident && <span style={{ fontSize: 15, color: C.mid }}>{resident.id}</span>}
        {resident && <button style={{ ...S.btn2, padding: "5px 10px", fontSize: 14 }} onClick={onLogout}>Выйти</button>}
      </div>
  ...
  return (
    <Shell>
      <form onSubmit={submit} style={{ ...S.card, maxWidth: 360, margin: "40px auto 0", width: "100%", boxSizing: "border-box" }}>
        ...
        <p style={{ fontSize: 14, color: C.sub, margin: "14px 0 0" }}>ID и PIN выдаёт руководитель программы.</p>
      </form>
    </Shell>
  );
}
  ...
  return (
    <Shell resident={resident} onLogout={onLogout}>
      <div style={S.card}>
        ...
        {CONSENT_TEXT.map((t, i) => <p key={i} style={{ fontSize: 16, lineHeight: 1.55, margin: "0 0 10px" }}>{t}</p>)}
        <label style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 16, margin: "14px 0", cursor: "pointer" }}>
        ...
        {home.items.map((i) => {
          ...
          return (
            <div key={i.case_id + i.mode} style={S.card}>
              <div style={{ fontSize: 14, color: C.sub }}>{i.date} · {modeName(i)}</div>
              <div style={{ fontSize: 18, fontWeight: 600, margin: "4px 0 8px" }}>{i.title}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <span style={{ fontSize: 14, color }}>{i.locked ? "Откроется после первого прохождения" : label}</span>
...
function Typing() {
  return <div style={{ color: C.sub, fontSize: 15, padding: "6px 2px" }}>Преподаватель пишет…</div>;
}
...
function Session({ initial, resident, onBack, onLogout }) {
  ...
  const bubble = (m) => ({
    ...
    borderRadius: 12, padding: "10px 14px", fontSize: 16, lineHeight: 1.55, whiteSpace: m.role === "user" ? "pre-wrap" : "normal",
  });

  return (
    <Shell resident={resident} onLogout={onLogout}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <button style={{ ...S.btn2, padding: "6px 10px", fontSize: 14 }} onClick={onBack}>← Задания</button>
        <span style={{ fontSize: 15, color: C.mid, flex: 1 }}>{attempt.title}</span>
      </div>
      ...
      {draft && <div style={{ fontSize: 14, color: C.sub, marginBottom: 8 }}>Кейс вымышленный. Не вводите данные реальных пациентов. Чтобы закончить, нажмите «Завершить кейс» или напишите «конец кейса».</div>}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, flex: 1, overflowY: "auto", paddingBottom: 8 }}>
        {attempt.transcript.filter((m) => m.kind !== "finish").map((m, i) => (
          <div key={i} style={bubble(m)}>
            {m.kind === "debrief" && <div style={{ fontSize: 13, color: C.mid, marginBottom: 4, letterSpacing: "0.06em" }}>РАЗБОР</div>}
      ...
      {attempt.status === "finished" && (
        <div style={{ ...S.card, marginTop: 10, background: C.warnBg }}>
          <div style={{ fontSize: 16, marginBottom: 10 }}>Кейс завершён. Нажмите кнопку, чтобы отправить вашу переписку руководителю программы для разбора.</div>
